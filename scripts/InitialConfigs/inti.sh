...
sudo sed -i '/^deb https:\/\/enterprise.proxmox.com\/debian\/pve bookworm pve-enterprise/ i #No subscription' "$pve_file_path"
sudo sed -i 's|^deb https://enterprise.proxmox.com/debian/pve bookworm pve-enterprise|#&|' "$pve_file_path"

# Insert "# disable enterprise" at the top and comment out the repository line
sudo sed -i '1i # disable enterprise' "$ceph_file_path"
sudo sed -i 's|^deb https://enterprise.proxmox.com/debian/ceph-quincy|#&|' "$ceph_file_path"
